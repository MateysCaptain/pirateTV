...
function NextPage() {
	...
	// console.log(id, name, eNo, sNo);
  	...
  	async function fetchData() {
		...
		if (response2.data.hasOwnProperty('number_of_seasons') && response2.data.number_of_seasons !== null) {
			...
			// const sno = response2.data?.seasons[1].season_number;
			// const sid = response2.data?.seasons[1].id;
	...
	const collectionClickHandler = (collectionId) => {
  	...
  	// console.log(text?.episode_run_time);

  	// console.log(similarMovies);
  	// console.log(epId, text?.id);
	...
	return (
		<div className="text-white" id="mpDiv1">
			{isLoading ? (
      		...
      		<>
				<header id="headerDiv" className="fade-in" style={{ backgroundImage: `linear-gradient(to bottom, rgba(0, 0, 0, 0) 20%, rgba(0, 0, 0, 0.93) 80%), url(https://image.tmdb.org/t/p/original${text?.backdrop_path || text?.still_path})` }}>
						...
						<div className="row">
							<div className="col-12 d-flex flex-row flex-wrap p-0" id="mpDiv4">
								...
								<div className="col-12 col-sm-12 col-md-6 p-2 mt-2 mb-2">
									...
									<div className="flex-item mb-3">
										<span style={{ color: 'grey' }}>Genres:</span> {genre && genre.map(i => (<span key={i.id} className="div_description1">{i.name}, </span>))}
									</div>
									<div className="flex-item mb-3">
